...
const getQuizExamById = async (id) => {
  const responseAPI = (await Axios({ url: `/exam/quiz/${id}`, method: "GET" }))
    .data;
  console.log(responseAPI);
  const formatData = {
    id: responseAPI.id,
    passMarks: responseAPI.pass_score,
    numberOfQuestions: responseAPI.questions.length,
    title: responseAPI.display_name,
    duration: responseAPI.duration,
    totalMarks: responseAPI.max_score,
    deadline: [responseAPI.startTime, responseAPI.endTime],
    questions: [],
  };
  formatData.questions = shuffleArray(
    responseAPI.questions.map((question) => {
      const formatQues = {
        id: question.id,
        title: question.question,
        level: question.level,
